...
class Support(Main):
                        ...
                        for j in range(0, len(support_child[i].children())):
                            ...
                            if text:
                                if text == "是否加固" or text == "支撑加底座" or text == "仅底座":
                                    ...
                                    s += 1
                                    support_static_dict[f"support{s}"] = support_child[i].children()[
                                        j].get_toggle_state()
                                else:
                                    s += 1
                                    support_static_dict[f"support{s}"] = support_child[i].children()[j].texts()[0]
    ...
    def choice_application(self, click_index, parent=2):
        ...
        :param parent: 通过第几层父级查找
        :return:
        """
        outside = self.find(auto_id=self.support_parent()[0].parent().parent().get_properties()["automation_id"])
        application_parent = self.support_parent(index=2)
        app = self.find(auto_id=application_parent, isall=False)
        self.is_in_outside(outside=outside, control=app)
    ...
    def support_set_button(self, oper, path=None, conffile=None, whether=True, isclose=False):
        ...
        :return:
        """
        oper_control = self.find(auto_id=self.support_parent(index=5), isall=False)
        outside = self.find(self.support_parent()[0].parent().parent())
        self.is_in_outside(outside=outside, control=oper_control)
        if oper == "保存":
            auto_id = self.support_parent(index=5)
            self.click(control=oper_control)
        ...
        try:
            for oper_key, oper_value in kwargs.items():
                ...
                if oper_key == "是否加固" or oper_key == "支撑加底座" or oper_key == "仅底座":
                    state = oper_control.get_toggle_state()
                ...
                for i in range(len(children)):
                    ...
                    if auto_id == oper_auto_id:
                        ...
                        outside = self.find(
                            auto_id=self.support_parent()[0].parent().parent().get_properties()["automation_id"])
                        self.is_in_outside(outside=outside, control=control)
                        # self.is_in_outside(outside=parent.parent().parent(),control=control)
                        self.insert(value=oper_value, control=control)
        except:
            LogRoot.error("参数有误，请检查！！")

    # 生成-删除-编辑
    def support_oper(self, oper="生成"):
        support_oper = self.find(index=oper)
        outside = self.find(auto_id=self.support_parent()[0].parent().parent().get_properties()["automation_id"])
        self.is_in_outside(outside=outside, control=support_oper)
        self.click(control=support_oper)
        LogRoot.info(oper)
        if oper == "生成":
            LogRoot.info("支撑生成进度条")
            return SupportFrame(self._dlg)
